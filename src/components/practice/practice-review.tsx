...
export function PracticeReview({ review }: { review: PracticeReviewData }) {
  return <div className="space-y-6">
    <Card><CardContent className="grid gap-3 pt-5 sm:grid-cols-3">
      ...
      <Metric label="Mistakes" value={String(review.summary.incorrect)} />
    ...
    <ol className="space-y-8">
      {review.items.map((item) => <li id={`question-${item.position}`} key={item.position}>
        <Card><CardHeader><p className="text-xs font-semibold uppercase tracking-wide text-primary">Question {item.position} · {item.question.difficulty}</p><CardTitle>{item.question.questionText}</CardTitle></CardHeader><CardContent className="space-y-5">
